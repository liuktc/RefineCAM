"""
Adapted from:
https://github.com/visinf/funnybirds/blob/main/funnybirds_complete/evaluation_protocols.py
"""
...
class Accuracy(BaseMetric):
    ...
    def __call__(self, model, test_dataset, device:str='cpu', **kwargs):
        class Summary(Enum):
            NONE = 0
            AVERAGE = 1
            SUM = 2
        ...
        for samples in test_loader:
            ...
            target = samples['class_idx'].to(device)
...
class ControlledSyntheticDataCheck(BaseMetric):
    def __init__(self):
        super().__init__("CSDC")
        pass
    
    def __call__(self, model, explainer, test_dataset, saliency_maps: torch.Tensor, device:str='cpu', **kwargs):
        ...
        for samples in test_loader:
            images = samples['image'].to(device)
            target = samples['class_idx'].to(device)
            part_maps = samples['part_map'].to(device)
            ...
            important_parts_for_thresholds = explainer.get_important_parts(images, part_maps, target, test_dataset.colors_to_part, thresholds=thresholds, attribution=saliency_maps)
...
class SingleDeletion(BaseMetric):
    def __init__(self):
        super().__init__("SD")
        pass
    
    def __call__(self, model, explainer, test_dataset, saliency_maps, device:str="cpu", **kwargs):
        ...
        for sample in test_loader:
            ...
            target = sample['class_idx'].to(device)
            part_map = sample['part_map'].to(device)
            ...
            part_importances = explainer.get_part_importance(image, part_map, target, test_dataset.colors_to_part, attribution=saliency_maps)
            score_diffs = {}
...
class PreservationCheck(BaseMetric):
    def __init__(self):
        super().__init__("PC")
        pass
    ...
    def __call__(self, model, explainer, test_dataset, saliency_maps, device:str = "cpu", **kwargs):
        ...
        for samples in (test_loader):
            ...
            important_parts_for_thresholds = explainer.get_important_parts(images, part_maps, model_prediction_original, test_dataset.colors_to_part, attribution=saliency_maps,thresholds=thresholds)
            for important_parts, threshold in zip(important_parts_for_thresholds, thresholds):
...
class DeletionCheck(BaseMetric):
    def __init__(self):
        super().__init__("DC")
        pass

    def __call__(self, model, test_dataset, explainer, saliency_maps, device:str = "cpu", **kwargs):
        test_loader = DataLoader(test_dataset, batch_size=1, shuffle=False)
...
class TargetSensitivity(BaseMetric):
    def __init__(self):
        super().__init__("TS")
        pass
    ...
    def __call__(self, model, test_dataset, explainer, saliency_maps, device:str = "cpu", **kwargs): 
        ...
        for sample in (test_loader):
            ...
            part_importances_class1 = explainer.get_part_importance(image, part_map, class1, test_dataset.colors_to_part, attribution=saliency_maps)
            part_importances_class2 = explainer.get_part_importance(image, part_map, class2, test_dataset.colors_to_part, attribution=saliency_maps)
...
class Distractibility(BaseMetric):
    def __init__(self):
        super().__init__("distractibility")
        pass
    ...
    def __call__(self, model, test_dataset, explainer, saliency_maps, device:str = "cpu", **kwargs): 
        ...
        for sample in (test_loader):
            ...
            explanation_important_parts_for_thresholds = explainer.get_important_parts(image, part_map, target, test_dataset.colors_to_part, with_bg=True, thresholds=thresholds, attribution=saliency_maps)
...
class BackgroundIndependence(BaseMetric):
    def __init__(self):
        super().__init__("background_independence")
        pass
